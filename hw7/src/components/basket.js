const basket = {
    ...
    basketBadge: null,
    basketTotal: null,
    url: 'https://raw.githubusercontent.com/bakla-zhan/static/master/JSON/basket.json',

    init() {
        this.wrapper = document.querySelector('.drop-cart');
        this.container = document.querySelector('#basket');
        this.basketBadge = document.querySelector('#cart-badge');
        this.basketTotal = document.querySelector('#basket-total');
        this._get(this.url)
            .then(basketObject => {
    ...
    _handleEvents() {
        ...
        this.container.addEventListener('click', e => {
            ...
            } else if (e.target.parentNode.name == 'remove') {
                this.remove(e.target.parentNode.dataset.id);
            }
        })
    },
    ...
    _render() {
        ...
        this.items.forEach((item) => {
            ...
            <div class="drop-cart__product">
                ...
                <a href="#" data-id="${item.productId}" name="remove" class="drop-cart__product-close"><i class="far fa-times-circle"></i></a>
            </div>`;
            total += item.productPrice * item.amount;
        });
        this.basketBadge.innerText = this.items.length;
        this.basketTotal.innerText = '$' + total;
